...
// }
    ...
    if( isset($_POST['sub']) ){
        ...
        if($erreur == ""){
            try {
            $req4 = $bdd->prepare("UPDATE technique SET Nom=:Nom,Famille=:Famille,Presentation=:Presentation,Conseils=:Conseils WHERE ID=:ID");
            ...
            $req4->bindValue('ID',$techniqueID, PDO::PARAM_INT);
            $req4->execute();
        ...
        //         ,mysql_real_escape_string($_POST['conseil'])
    ...
    try {
            ...
            $req->bindValue('ID',$techniqueID, PDO::PARAM_INT);
            $req->execute();
            $technique=$req->fetch(PDO::FETCH_ASSOC);
            $req2=$bdd->prepare("SELECT * FROM technique_specialiste WHERE id_technique LIKE :ID order by nom_champion");
            $req2->bindValue('ID',$techniqueID, PDO::PARAM_INT);
            $req2->execute();
            $res_spec= array();
            while ( $row  = $req2->fetch(PDO::FETCH_ASSOC)) {  
                array_push($res_spec, $row);
            }
        ...
        <fieldset>
					...
					if(isset($_POST['sup_spec'])){
						for($i=0,$n=count($_POST["j_sup"]);$i<$n;$i++){
                            try {
                                $req3 = $bdd->prepare("DELETE FROM technique_specialiste WHERE id_champion=:ID");

                                $req3->bindValue('ID',$_POST["j_sup"][$i], PDO::PARAM_INT);
                                $req3->execute();
					...
					elseif(isset($_POST['ajout_special'])){
                        try {
                                $req4 = $bdd->prepare("INSERT INTO technique_specialiste VALUES('',:techniqueID,:j_idchamp,:j_name)");

                                $req4->bindValue('techniqueID',$techniqueID, PDO::PARAM_STR);
                                $req4->bindValue('j_idchamp',$_POST['j_idchamp'], PDO::PARAM_STR);
                                $req4->bindValue('j_name',$_POST['j_name'], PDO::PARAM_STR);
                                $req4->execute();
					...
					elseif(isset($_POST['special'])){
						echo "<h1>Ajouter un sp&eacute;cialiste</h1>";
					...
					elseif(isset($retour)){ echo '<span style="color:green;">Modification r&eacute;ussie</span><br /><br />'; }else{?>
					...
					?>
                        <div ><input type="submit" name="special" value="Ajouter des sp&eacute;cialistes"/><?php if(sizeof($res_spec)!=0){echo '<input type="submit" name="sup_spec" value="Supprimer les sp&eacute;cialistes"/>';} ?><input type="submit" name="sub" value="Modifier"/></div>
                </form><div><?php } ?>
                </fieldset>
    </body>
</html>