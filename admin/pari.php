...
if( isset($_POST['sub'])) {
    ...
    if($erreur == "" ) {
        try {
            $req = $bdd->prepare("INSERT INTO pari (date_debut,date_fin,actif,description,evenement_id) VALUES (:date_debut,:date_fin,:actif,:description,:evenement_id)");
...
try{
  $req1 = $bdd->prepare("SELECT p.*, e.Nom FROM pari p INNER JOIN evenements e ON p.evenement_id = e.id ORDER BY ID DESC ");
  $req1->execute();
  $result1= array();
