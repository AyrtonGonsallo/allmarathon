...
function display_rss($result1){ 
    ...
    foreach ($result1 as $res) {
        ...
        $loc        = 'https://dev.allrathon.fr/actualite-marathon-'.$res['ID'].'-'.slugify($url_text).'.html';
        $date = $res['date'];
        $titre = $res['titre'];
        $tab = explode('-',$date);
        $yearNews  = $tab[0];
        $img_src='/images/news/'.$yearNews.'/'.$res['photo'];
        $photo = 'https://dev.allrathon.fr'.$img_src;
        $source = $res['source'];
...
// Affichage
    ...
    <atom:link href="https://dev.allrathon.fr/flux-rss.xml" rel="self" type="application/rss+xml" />
