...
function display_sitemap_evenements($result1){ 
    // Boucle qui liste les URL
    foreach ($result1 as $res) {
        $nom_res_lien_archive=' - '.$res['Nom'].' - '.utf8_encode(strftime("%A %d %B %Y",strtotime($res['DateDebut'])));
        $loc        = 'https://www.allmarathon.fr/resultats-marathon-'.$res['ID'].'-'.slugify($nom_res_lien_archive).'.html';
        echo '
        <url>
            <loc>'.$loc.'</loc>
        </url>';
