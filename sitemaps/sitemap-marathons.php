...
function get_data_sitemap_marathons(){
    // Récupération des données dans la base de données (MySQL)
    require_once '../database/connexion.php';
    $result1= array();
    try{
...
function display_sitemap_marathons($result1){ 
    // Boucle qui liste les URL
    foreach ($result1 as $res) {
        
        $loc        = 'https://dev.allmarathon.fr/marathons-'.$res['id'].'-'.slugify($res['nom']).'.html';
        echo '
        <url>
            <loc>'.$loc.'</loc>
        </url>';
...
// Affichage
    ...
    <?php display_sitemap_marathons(get_data_sitemap_marathons()); ?>
    </urlset>
