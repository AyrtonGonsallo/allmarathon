  ...
  (function ($) {
    $.extend($.fn, {
          ...
          $(this).each(function () {
            ...
            if ($.browser.msie) {
              flashElement =
                ...
                '" id="' +
                $(this).attr("id") +
                'Uploader" class="fileUploaderBtn">\
            ...
            if (settings.onInit() !== false) {
              ...
              if ($.browser.msie) {
                $(this).after(
                  '<div id="' + $(this).attr("id") + 'Uploader"></div>'
                );
                document.getElementById(
                  $(this).attr("id") + "Uploader"
                ).outerHTML = flashElement;
              } else {
                ...
                if (event.data.action(event, queueID, fileObj) !== false) {
                  ...
                  $("#" + $(this).attr("id") + "Queue").append(
                    '<div id="' +
                      ...
                      $(this).attr("id") +
                      queueID +
                      'ProgressBar" class="fileUploadProgressBar" style="width: 1px; height: 3px;"></div>\
									</div>\
								</div>'
                  );
                }
                  ...
                  function (data) {
                    for (var key in data) {
                      if (
                        ...
                        var replaceFile = confirm(
                          "Do you want to replace the file '" + data[key] + "'?"
                        );
                        if (!replaceFile) {
                          document
                            .getElementById(
                              $(event.target).attr("id") + "Uploader"
                            )
                            .cancelFileUpload(key);
                        }
                      }
                    }
                    if (single) {
                      document
                        .getElementById($(event.target).attr("id") + "Uploader")
                        .startFileUpload(singleFileID, true);
                    } else {
                      document
                        .getElementById($(event.target).attr("id") + "Uploader")
            ...
            $(this).bind(
              ...
              function (event, queueID, fileObj, data) {
                if (
                  event.data.action(event, queueID, fileObj, data) !== false
                ) {
                  $("#" + $(this).attr("id") + queueID).fadeOut(
                    250,
                    function () {
            ...
            $(this).bind(
              ...
              function (event, queueID, fileObj, errorObj) {
                if (
                  event.data.action(event, queueID, fileObj, errorObj) !== false
                ) {
                  $("#" + $(this).attr("id") + queueID + " .fileName").text(
                    errorObj.type + " Error - " + fileObj.name
                  );
                  $("#" + $(this).attr("id") + queueID).css({
            ...
            $(this).bind(
              ...
              function (event, queueID, fileObj, data) {
                if (
                  ...
                  $("#" + $(this).attr("id") + queueID + "ProgressBar").css(
                    "width",
                    data.percentage + "%"
                  );
                  ...
                  $("#" + $(this).attr("id") + queueID + " .percentage").text(
                    displayData
                  );
                }
              }
      ...
      fileUploadSettings: function (settingName, settingValue) {
        $(this).each(function () {
          document
            .getElementById($(this).attr("id") + "Uploader")
